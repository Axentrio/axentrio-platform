...
export const getSetupProgress: CopilotTool<Record<string, never>, SetupProgressResult> = {
  ...
  description:
    "Return how far the tenant got through first-run setup: whether it is complete, which step is outstanding, and which steps they answered vs explicitly skipped. Steps are: language, company, logo, chatbot, documents, plan, bookings, leads, social. A SKIPPED step is a deliberate decision by the customer — do not nag about it; mention it only if they ask about that feature. If grandfathered is true the workspace predates the setup wizard and was completed automatically: it never walked the steps, so notYetReached lists all of them and means NOTHING — do not report it, do not tell them they missed anything, and treat setup as done. Never say setup is complete and in the same breath that steps were not reached. Use this before suggesting setup work, so you never tell someone to do something they have already done.",

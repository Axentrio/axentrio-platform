...
describe('block ledger — agent mode', () => {
  ...
  it('SKILL_PROSE_<id>: emits a bound skill\'s prose (override or default) as its own block', () => {
    const out = composeSystemPrompt({ ...base, tools: [], skillProse: [{ id: 'booking', prose: 'Custom booking guidance for this template.' }] });
    expect(out.ledger.getIncluded()).toContain('SKILL_PROSE_booking');
    expect(out.prompt).toContain('Custom booking guidance for this template.');
  });
});

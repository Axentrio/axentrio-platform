...
describe('the counterfactual order', () => {
  ...
  it('sorts scored slots by cost and keeps neutrals chronological after them', () => {
    const order = counterfactualOrder({
      scored: [slot('11:00', 90), slot('08:00', null), slot('09:00', 5)],
      requestable: [],
    });
    expect(order).toEqual([iso('09:00'), iso('11:00'), iso('08:00')]);
  });

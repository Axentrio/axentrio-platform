...
const qualityConfig: Record<string, { color: string; labelKey: string }> = {
    excellent: {
        ...
        labelKey: "ai.knowledge.card.quality.excellent",
    },
    good: {
        color: "bg-emerald-400",
        labelKey: "ai.knowledge.card.quality.good",
    },
...
const typeConfig: Record<
    string,
    { icon: React.ElementType; labelKey: string; accent: string }
> = {
    pdf: {
        icon: FileText,
        labelKey: "ai.knowledge.card.type.pdf",
        accent: "text-rose-400 bg-rose-400/10",
    },
    docx: {
        icon: FileText,
        labelKey: "ai.knowledge.card.type.docx",
        accent: "text-blue-400 bg-blue-400/10",
    },
    text: {
        icon: FileEdit,
        labelKey: "ai.knowledge.card.type.text",
        accent: "text-violet-400 bg-violet-400/10",
    },
    faq: {
        icon: HelpCircle,
        labelKey: "ai.knowledge.card.type.faq",
        accent: "text-amber-400 bg-amber-400/10",
    },
    url: {
        icon: Globe,
        labelKey: "ai.knowledge.card.type.url",
...
const statusConfig: Record<
    string,
    { dot: string; labelKey: string; bg: string }
> = {
    indexed: {
        dot: "bg-emerald-400",
        labelKey: "ai.knowledge.card.status.indexed",
        bg: "bg-emerald-400/10 text-emerald-400",
    },
    processing: {
        dot: "bg-amber-400 animate-pulse",
        labelKey: "ai.knowledge.card.status.processing",
        bg: "bg-amber-400/10 text-amber-400",
    },
    pending: {
        dot: "bg-text-muted",
        labelKey: "ai.knowledge.card.status.pending",
        bg: "bg-surface-3 text-text-muted",
    },
    failed: {
        dot: "bg-red-400",
        labelKey: "ai.knowledge.card.status.failed",
        bg: "bg-red-400/10 text-red-400",
    },
};
                ...
                <div className="mt-3 flex items-start gap-2 p-2 bg-red-400/5 rounded-lg border border-red-400/10">
                    ...
                    {isAdmin && (
                        <button
                            ...
                            title={t(
                                "ai.knowledge.card.actions.retryProcessing",
                            )}
                        >

...
// Atomic counter advance — mirrors advanceLoopState(): human ⇒ reset streaks;
// identical substantive message ⇒ repeated++; repeat OR non-meaningful ⇒
// botLike++; suspicious link ⇒ susp++. Returns [repeated, botLike, susp].
const ADVANCE_LUA = `
local k = KEYS[1]
local hash = ARGV[1]
local meaningful = ARGV[2] == '1'
local human = ARGV[3] == '1'
local hasLink = ARGV[4] == '1'
...
if human then
  ...
  local substantiveRepeat = isRepeat and meaningful
  if substantiveRepeat then repeated = repeated + 1 elseif meaningful then repeated = 1 else repeated = 0 end

...
**Delivered in three waves** — one shared harness, 12 vitest files holding 96 passing tests, and one
...
| File | Tests | Closes |
|---|---|---|
| `src/__tests__/integration/booking-plan-truth-guard.test.ts` | 13 | **SYS-07** and **CAL-06** (reply half), one root cause: run state that said something happened when it had not. Changes `agent/agent.service.ts`, `agent/tools/capture-lead.tool.ts` and `guardrails/output-validation.ts` |

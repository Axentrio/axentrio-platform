...
/**
 * Chip labels for the callback offer, in the session's language.
 *
 * Hardcoded per language rather than translated at runtime: these two strings ARE the
 * consent prompt, so they must be reviewable and stable, not regenerated by a model.
 * Falls back to English for any language not listed.
 */
function callbackLabels(session: ChatSession): { yes: string; no: string } {
  const lang = (session.metadata?.customData?.language as string | undefined)?.slice(0, 2)?.toLowerCase();
  switch (lang) {
    case 'nl':
      return { yes: 'Ja, bel me terug', no: 'Nee, bedankt' };
    case 'fr':
      return { yes: 'Oui, rappelez-moi', no: 'Non merci' };
    default:
...
export class AgentService {
  ...
  async run(
    ...
    try {
      ...
      // Did the customer actually describe a need this run? The callback offer is only
      // allowed once they have — appending it to an FAQ answer is the forced-form feel
      // Story 3 forbids.
      let capturedRequestThisRun = false;
      ...
      for (let i = 0; i < MAX_ITERATIONS; i++) {
        ...
        if (response.finishReason === 'stop' || !response.toolCalls?.length) {
          ...
          trace.finishReason = 'completed';
          void this.traceLogger.save(trace); // fire-and-forget: keeps the trace write off the response path
          // Booking slots take precedence: when the customer is mid-booking we are
          // already collecting their details, so a callback offer would be noise.
          const slotChips = buildSlotQuickReplies(pendingAvailability);
          return {
            type: 'response',
            content: finalContent,
            quickReplies: slotChips ?? (await this.maybeCallbackOffer(session, capturedRequestThisRun)),
          };
        }
        ...
        for (const toolCall of response.toolCalls) {
          ...
          try {
            ...
            } else if (tool.name === 'capture_lead' && result.success) {
              // Arms the callback offer: the model judged there was a real request
              // worth saving, which is a better signal than any turn counter.
              capturedRequestThisRun = true;
            }
  ...
  /**
   * The Pro proactive-capture offer (Story 3): two tappable chips appended to an
   * ordinary reply when we still have no way to reach a customer who has just described
   * a real need.
   *
   * Deliberately NOT an LLM ask. The bot's wording is untouched, so "never pushy" is a
   * structural property rather than prompt hope — and a tap is explicit, storable
   * consent in a way that a model coaxing out a phone number is not. Every restraint
   * lives in `shouldOfferCallback`; this method only gathers the inputs.
   *
   * Fails CLOSED: any error means no chips. An offer is a nice-to-have, and a broken
   * entitlement lookup must never cause the bot to solicit personal data.
   */
  private async maybeCallbackOffer(
    session: ChatSession,
    capturedRequestThisRun: boolean,
  ): Promise<QuickReply[] | undefined> {
    if (!capturedRequestThisRun) return undefined;
    try {
      const { features } = await getEntitlements(session.tenantId);
      // Effective value: entitled AND explicitly switched on (the toggle is opt-in).
      if (features.proactiveLeadCapture !== true) return undefined;

      // Do we already have an off-channel contact for this person? On a messaging
      // channel we can always reply, so a callback number adds little and the offer
      // would read as pushy.
      const isChannel = !!session.channel && session.channel !== 'widget';
      let hasContact = isChannel;
      if (!hasContact) {
      ...
      const state = readOfferState(session);
      const labels = callbackLabels(session);
      if (
        !shouldOfferCallback({
          enabled: true,
          hasContact,
          requestCaptured: capturedRequestThisRun,
      ...
      // Record the offer BEFORE returning it, so a retry or a parallel turn cannot
      // produce a second one. At most one offer per conversation, ever.
      await AppDataSource.query(
        `UPDATE chat_sessions SET metadata = $2::jsonb, updated_at = now() WHERE id = $1`,
        [session.id, JSON.stringify(withOfferState(session.metadata as Record<string, unknown>, { offeredAt: new Date().toISOString() }))],
      );
      return buildCallbackQuickReplies(labels);
    } catch (error) {
      logger.warn('[agent] callback offer skipped', {
        sessionId: session.id,

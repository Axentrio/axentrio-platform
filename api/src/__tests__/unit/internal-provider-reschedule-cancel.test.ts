...
describe('InternalProvider.rescheduleBooking — travel', () => {
  ...
  const ACTIVE = { active: true as const, tenantId: 'ten-1', itineraryKey: 'bot:bot-1', slackMin: 0, startFromBase: false, maxDetourMin: null, baseDepartOffsetMin: 0, preferClusters: false };
...
describe('InternalProvider.rescheduleBooking — what the move exposed', () => {
  ...
  const MOBILE = { ...EVENT_TYPE, customerAddressRequired: true };
  const PLACE = { placeId: 'ChIJ_p', lat: 51.05, lng: 3.72, precision: 'rooftop' as const, formattedAddress: 'Kerkstraat 12, 9000 Gent, Belgium' };
  const BASED = { active: true as const, tenantId: 'ten-1', itineraryKey: 'bot:bot-1', slackMin: 0, startFromBase: true, maxDetourMin: null, baseDepartOffsetMin: 0, preferClusters: false };

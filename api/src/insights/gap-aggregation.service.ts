...
/** Apply this run's window to an existing gap, moving it along the lifecycle. */
function applyWindowToGap(
  tenantId: string,
  gap: Gap,
  stats: TopicWindowStats,
  now: Date,
): void {
  ...
  const qualifyingPain = hasQualifyingPain(stats);
 ...
 */
async function sweepDormantGaps(
  gapRepo: Repository<Gap>,
  existingGaps: Gap[],
  ...
  for (const gap of existingGaps) {
    ...
    const qualifyingPain = hasQualifyingPain(byTopic.get(gap.canonicalTopicId));
...
export async function aggregateGaps(tenantId: string, now: Date): Promise<void> {
  ...
  for (const stats of byTopic.values()) {
    ...
    if (!gap) {
      ...
      if (hasQualifyingPain(stats)) {
        await gapRepo.save(
          gapRepo.create({
            tenantId,
            canonicalTopicId: stats.canonicalTopicId,
            status: 'open',
            severity: stats.unsatisfiedVisitors.size >= RED_VISITORS ? 'red' : 'orange',
    ...
    applyWindowToGap(tenantId, gap, stats, now);
    await gapRepo.save(gap);
  }

  const dormancyCutoff = new Date(now.getTime() - DORMANCY_DAYS * 24 * 60 * 60 * 1000);
  await sweepDormantGaps(gapRepo, existingGaps, byTopic, dormancyCutoff);
}

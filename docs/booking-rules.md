...
Pinned: `booking-prompt-behaviour.test.ts` (notice/horizon, daily cap, closed weekday, out-of-hours, check-before-capture, phone). Engine: `diagnoseEmptyRange` in `slot-engine.ts`. Tool: `outOfWindowGuidance` / empty-range `suggestedAction: 'check_availability'`. `internal-provider-create.test.ts` (check before capture). The three refusals above, on the WRITE path and against a real Availability Rule row: `integration/booking-plan-hours-gate.test.ts` — an out-of-hours hour keeps the named date while that date still has a time the business can take and moves to another date when it has none, a closed date moves to another date, `too_soon` gets a reachable range, and the daily cap outranks all three. A reschedule change Request at an out-of-hours hour gets the same refusal. The hours check itself is `isWithinBusinessHours` in `slot-engine.ts`, shared with the offer path. A window that closes at or before it opens counts as no hours (`dayHasHours`), so that date gets the closed-day answer.

...
/** Name, channel and the best available way to reach one lead. */
function LeadPersonCell({ lead }: { lead: Lead }) {
...
/**
 * Days, not a timestamp: the question is "how long have they been waiting", and a
 * date makes the reader do that subtraction.
 */
function LeadWaitingCell({ lead, days }: { lead: Lead; days: number | null }) {
  const { t } = useTranslation();
  return (
...
/** The two `leadEnrichment` columns: the recommendation and the booking state. */
function LeadEnrichmentCells({ lead }: { lead: Lead }) {
  const { t } = useTranslation();
  return (
    <>
...
/** Open/handled badge plus when the lead arrived. */
function LeadStatusCell({ lead }: { lead: Lead }) {
  const { t } = useTranslation();
  return (
    <TableCell>
...
/** Contact, handled/reopen, erase, and the expand chevron. */
      ...
      <div className="flex items-center justify-end gap-1">
        ...
        {(lead.phone || lead.email) && (
          <a
            ...
            className="rounded p-2 hover:bg-surface-3 hover:text-primary-400 focus-visible:text-primary-400"
          >
            {lead.phone ? (
        ...
        <button
          ...
          className="rounded p-2 hover:bg-surface-3 hover:text-text-secondary disabled:opacity-50"
        ...
        <button
          ...
          className="rounded p-2 hover:bg-surface-3 hover:text-destructive disabled:opacity-50"
...
function LeadRow({
  ...
  return (
    <TableRow
      className={`cursor-pointer ${severityClass(lead)} ${lead.status === 'archived' ? 'opacity-50' : ''}`}
      onClick={onToggle}
    >
      <LeadPersonCell lead={lead} />
      ...
      {hasEnrichment && <LeadEnrichmentCells lead={lead} />}
...
/** The expanded drawer row under a lead. */
function LeadDetailRow({
  ...
  const { t } = useTranslation();
  return (
    <TableRow className="bg-surface-2/30 hover:bg-surface-2/30">
                ...
                {allLeads.map((lead) => {
                  ...
                  return [
                    <LeadRow
                      ...
                      erasePending={eraseLead.isPending}
                      onToggle={() => setExpanded((s) => ({ ...s, [lead.id]: !s[lead.id] }))}
                      onToggleStatus={() =>
                        updateStatus.mutate({
                          id: lead.id,

...
export function createWebhookRouter(config: WebhookRoutesConfig): Router {
  ...
  // ============================================================================
  // Outbound Webhook Endpoints (For n8n to register callbacks)
  // ============================================================================

  /**
   * @route   POST /api/v1/n8n/webhook/register
   * @desc    Register a new webhook URL for receiving events
   * @access  Admin
   * 
   * Body:
   * {
  ...
  /**
   * @route   DELETE /api/v1/n8n/webhook/unregister/:webhookId
   * @desc    Unregister a webhook
   * @access  Admin
   */
  router.delete('/unregister/:webhookId', async (req, res) => {
    try {
      const { webhookId } = req.params;
      // Implementation would remove webhook from database
      res.status(200).json({
        success: true,
  ...
  /**
   * @route   GET /api/v1/n8n/webhook/registered

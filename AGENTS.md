...
Run `npm test` in `api/` as ONE process. `src/__tests__/worker-database.ts` derives a single
fixed template database name, so two concurrent vitest runs destroy each other's template.

...
beforeEach(() => {
  send.mockReset();
  send.mockResolvedValue({ success: true, messageId: 'socket-handoff-email-1' });
});
...
function waitForEvent(client: ClientSocket, event: string, ms = 2000): Promise<unknown> {
...
describe('socket handoff:request notifications', () => {
  it('writes outbox + platform notification + email like the REST paths', async () => {
    ...
    try {
      ...
      client.emit('handoff:request', { sessionId: session.id, reason: 'User requested' });
      expect(await ack).toMatchObject({ sessionId: session.id, status: 'pending' });

      const handoff = await AppDataSource.getRepository(HandoffRequest).findOneByOrFail({

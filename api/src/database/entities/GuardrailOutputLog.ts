 ...
 * reply text — a leaked_internals hit may contain a real secret/API key, and
 * for shadow-mode FP tuning the persisted message is reachable via
 * outboundMessageId anyway.
 */
...
export class GuardrailOutputLog {
  ...
  /** The outbound message that was flagged. Null when an enforced block replaced
   *  the reply before it was ever persisted. */

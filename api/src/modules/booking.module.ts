...
6. If create_booking returns CAPACITY_REACHED, that time cannot be taken — the day is full, the business has no working time left that day, or the slot sits too close to another appointment. Read the message, offer a different time or the next available day, and do NOT retry the same one. Never tell the customer the business is closed.`
    ...
    hasDuration
      ? `
7. For a service shown with a duration RANGE (e.g. "30-90 min"), establish the length FIRST — ask the customer how long they need ("choose length"), or estimate it from what they have described ("AI-estimated") — then pass that as durationMin to check_availability AND the booking tool (same value). If a tool returns DURATION_REQUIRED, the length is not yet known: ask the customer for a number of minutes in the shown range, then re-call the tool. Never describe DURATION_REQUIRED as a technical problem or a calendar failure, and never capture a request in place of asking. Never call create_booking for one of these without a durationMin. If you genuinely cannot tell after asking, say so and capture it with request_appointment. If a tool returns DURATION_OUT_OF_RANGE, pick a length within the shown range. If create_booking returns SLOT_UNAVAILABLE for a range service, the chosen length didn't fit that start — offer a different start or a shorter length within range; don't retry the same start+length.`
      : ''
  }
